...
//  
//
//  Created by Caio Soares on 16/04/23.
//
...
class horiLine_Gesture: UIGestureRecognizer {
	...
	override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent) {
		...
		let _ 	= (newTouch?.previousLocation(in: self.view))!
		
		if self.strokePhase == .initialPoint {
			if initialTouchPoint.x < newPoint.x {
